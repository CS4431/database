...
class TestCourse < SinatraTest
  ...
  def test_return_all_course_information
    ...
    assert(data[0]['data'].has_key?('code'))
    assert(data[0]['data'].has_key?('section'))
    assert(data[0]['data'].has_key?('department_id'))
    assert(data[0]['data'].has_key?('instructor'))
    assert(data[0]['data'].has_key?('term'))
    assert(data[0]['data'].has_key?('department_name'))
  end
